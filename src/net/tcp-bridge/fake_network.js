// Vendored from v86 (https://github.com/copy/v86) src/browser/fake_network.js,
// BSD-2-Clause — see LICENSE.v86. Local modifications are marked "LOCAL PATCH".
// LOCAL PATCH: v86-internal imports replaced with a local shim module.
import { LOG_FETCH, h, dbg_assert, dbg_log } from "./shims.js";
 ...
 * @param {Object} spec
 */
// LOCAL PATCH: exported (with parse_eth) so tests can encode/decode frames.
export function make_packet(out, spec)
{
    dbg_assert(spec.eth);
...
function handle_fake_dns_static(packet, adapter)
{
    ...
    for(let i = 0; i < packet.dns.questions.length; ++i) {
        ...
        switch(q.type){
            case 1: { // A record
                // LOCAL PATCH: let the adapter map hostnames to distinct fake
                // IPs (returns a 4-element array, or null for "no answer").
                // Without the hook, this preserves v86's fixed answer.
                const answer_ip = adapter.dns_resolve
                    ? adapter.dns_resolve(q.name)
                    : [192, 168, 87, 1];
                if(!answer_ip) break;
                answers.push({
                    name: q.name,
...
export function parse_eth(data, o) {
